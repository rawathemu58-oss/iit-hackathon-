...
# ----------------------------------------------------------------------
# 4. MODEL — ensemble of HistGradientBoosting + RandomForest, predicting
#    post_hire_score. Feature set includes recruiter-note sentiment and
#    awards on top of the core numeric/behavioural signals.
# ----------------------------------------------------------------------
...
rf = RandomForestRegressor(n_estimators=500, max_depth=10, random_state=SEED, n_jobs=-1)
rf.fit(X_train_imp, y_train)

# weighted ensemble: HistGB carries more weight (handles missing values natively
# and generalized better in validation), RandomForest adds robustness
ENSEMBLE_WEIGHT_HGB = 0.7


def predict_ensemble(df):
    X = df[FEATURES]
    p_hgb = hgb.predict(X)
    p_rf = rf.predict(imputer.transform(X))
    return ENSEMBLE_WEIGHT_HGB * p_hgb + (1 - ENSEMBLE_WEIGHT_HGB) * p_rf
